defmodule NoozoWeb.Admin.Todo.Components.ItemModal.Content do
  ...
  def render(assigns) do
    ...
    <div id={@id}>
      <%= if @editing do %>
        <form class="" phx-target={@myself} phx-submit="update_content">
          <div class="flex flex-col gap-6">
            <textarea class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 w-full h-64 sm:text-sm border-gray-300 rounded-md"
                   type="text" name="content" data-component={@id}
                   id={@id}><%= @item.content %></textarea>

            <div class="flex flex-row gap-2">
              <div class="flex-grow">
                <input type="submit" class="tag-xs text-xs cursor-pointer" value="Update" />
              </div>
              <div phx-click="cancel" phx-target={@myself} class="tag-xs text-xs is-small is-danger is-right cursor-pointer">
